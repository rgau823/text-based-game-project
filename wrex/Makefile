...
.PHONY: memcheck
memcheck: $(GTEST)
	valgrind --tool=memcheck --leak-check=yes --track-origins=yes --error-exitcode=1 $(GTEST)

.PHONY: wrexmemcheck
wrexmemcheck: $(PROJECT)
	valgrind --tool=memcheck --leak-check=full --track-origins=yes --error-exitcode=1 $(PROJECT)

.PHONY: fullmemcheck
fullmemcheck: $(GTEST)
	valgrind --tool=memcheck --leak-check=full --track-origins=yes --error-exitcode=1 $(GTEST)
